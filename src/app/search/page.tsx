...
export default function StoreProductpage() {
  ...
  const fn_selectCategory = (category: any) => {
    setSelectedCat(category.slug);
  };

  const onFilter = async () => {
    console.log("filter", brandId, rating, selectedCat, price);
    let query = selectedCat
      ? `?category=${selectedCat}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${rating}`
      : `?price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${rating}`;

    if (brandId) {
      query = selectedCat
        ? `?brand_id=${brandId}&category=${selectedCat}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${rating}`
        : `?price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${rating}&brand_id=${brandId}`;
    }
